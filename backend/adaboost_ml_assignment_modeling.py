# -*- coding: utf-8 -*-
"""Copy of AdaBoost - ML Assignment Modeling.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1FMshTPOA0_W1FSFyCQI10ZaR8mcZjWEF
"""
...
AdaBoost (Adaptive Boosting) is an ensemble technique that combines multiple weak learners (usually decision trees) to create a strong learner. It adjusts weights iteratively to focus on difficult-to-predict instances.
"""

from sklearn.ensemble import AdaBoostRegressor

# Create the full pipeline for AdaBoost
...
"""### Evaluation (AdaBoost)
Calculating MAE, RMSE, and R2 Score for AdaBoost.
"""
...
y_pred_actual_ada = np.expm1(y_pred_ada)

# Calculate Metrics
mae_ada = mean_absolute_error(y_test_actual, y_pred_actual_ada)
mse_ada = mean_squared_error(y_test_actual, y_pred_actual_ada)
rmse_ada = np.sqrt(mse_ada)
r2_ada = r2_score(y_test_actual, y_pred_actual_ada)
...
plt.scatter(y_test_actual, y_pred_actual_ada, alpha=0.5, color='orange')
plt.plot([y_test_actual.min(), y_test_actual.max()],
         [y_test_actual.min(), y_test_actual.max()],
         'r--', lw=2)
plt.xlabel('Actual Price')
plt.ylabel('Predicted Price')
plt.title('Actual vs Predicted Car Prices (AdaBoost)')
plt.show()
...
column_transformer = adaboost_pipeline.named_steps['column_transform']
...
X_test_processed = adaboost_pipeline.named_steps['preprocessing'].transform(X_test)
X_test_processed = adaboost_pipeline.named_steps['column_transform'].transform(X_test_processed)
...
# Get the model from pipeline (the AdaBoostRegressor)
model = adaboost_pipeline.named_steps['model']
...
X_train_processed = adaboost_pipeline.named_steps['preprocessing'].transform(X_train)
X_train_processed = adaboost_pipeline.named_steps['column_transform'].transform(X_train_processed)
...
#   is the AdaBoostRegressor itself, we use 'model.predict'.
...
print("✅ SHAP explainer (KernelExplainer) created!")
...
dump_path = 'data/models/car_price_predictor_adaboost.pkl'
# Save the entire pipeline (Preprocessor + XGBoost)
joblib.dump(adaboost_pipeline, dump_path)

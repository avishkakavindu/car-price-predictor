# -*- coding: utf-8 -*-
"""ML Assignment Modeling.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1XtiCshR3hn5X3suABlJVavsW0YFOkcVf
"""
...
df = pd.read_csv('data/datasets/car_price_dataset.csv')
...
"""# Evaluation
According to the nature of the target variable and the business objective. Since we are predicting a continuous value (Price) we have to use:


- Mean Absolute Error:

  *Average magnitude of errors between predicted and actual values, calculated by averaging the absolute differences (ignoring sign) for all data points*

- Root Mean Squared Error:

   The average squared difference between predicted values from a model and the actual observed values.

   Unlike MAE, RMSE squares the errors before averaging them. This means large mistakes are penalized much more heavily than small ones.

- **R-squared Score**:

  *Indicate the percentage of the dependent variable's variance explained by the independent variable(s), showing how well your model fits the data (0 to 1, higher is better)*
...
- **$R^2$ score**: 0.8795: since it is closer to 1, it indicate model is highly effective.
- **MAE: 7.57**: If your price was in lakhs, this means on average, your prediction is off by Rs. 572,000.00. Basically model predicted can be wrong by this number.
- **RMSE: 15.78**:  Higher than the MAE. Means, while model usually accuarate by MAE(7.57), it makes a few larger errors on certain vehicles. There are outliers that confuses the model.

**Summary**:

The results indicate a highly robust model ($R^2 \approx 0.90$) that is generally accurate for the majority of the dataset (MAE = 7.57). However, the gap between MAE and RMSE suggests that while the XGBRegressor handles non-linear patterns well, it remains sensitive to price outliers. This mirrors the findings in financial risk literature where ensemble methods capture broad trends effectively but require further tuning to minimize variance in extreme 'high-risk' cases.
...
column_transformer = full_pipeline.named_steps['column_transform']
...
print(f"\nFeature breakdown:")
print(f"  • Numerical (scaled): {len(num_engine_features) + len(num_mileage_features)}")
print(f"  • Binary (passthrough): {len(binary_features)}")
print(f"  • Categorical (one-hot): {len(categorical_features_encoded)}")
...
X_test_processed = full_pipeline.named_steps['preprocessing'].transform(X_test)
X_test_processed = full_pipeline.named_steps['column_transform'].transform(X_test_processed)

# Convert to DataFrame with feature names
X_test_processed_df = pd.DataFrame(X_test_processed, columns=all_feature_names)

print(f"✅ Transformed shape: {X_test_processed_df.shape}")
...
print("\n🔍 Calculating SHAP values (using 100 samples)...")
...
Real Example:
- Low mileage (50k km)    → Push price UP
- Automatic               → Push price UP
- Large engine (2000cc)   → Push price UP
"""
...
1. This also says Mileage contrbute more to the price more than any other features, to be exact 22.3% importance.
  ```
  Real Example:
  Car A: 50,000 km  →  Price: 60 lakhs
  Car B: 200,000 km →  Price: 40 lakhs
...
Started with 40 lakh(average car price) then,
 ...
 -0.15 Nissan Brand price reduction → ~ 38.5 lakh
 +0.14 Engine CC price increase → ~39.9 lakh
 +0.11 Gear automatic trsansmission → ~50 lakh
 -0.06 Mileage not low mileage → ~49.4lakh
 -0.05 Not a Toyota brand → ~48.9 lakh
 +0.03 Not a Zuzuki brand → ~49.2 lakh
 +0.02 Fuel Group Petrol → ~49.4 lakh
 +0.02 Power mirror → ~49.6 lakh
 +0.02 Not a Micro brand → ~49.8 lakh
"""
...
joblib.dump(full_pipeline, 'data/models/car_price_predictor_xgbr.pkl')
print("Model saved to data/models/car_price_predictor_xgbr.pkl")

""" --- Later, in a different script or server ---

# Load the single file
model_service = joblib.load('data/models/car_price_predictor_xgbr.pkl')
...
# Load pipeline
pipeline_loaded = joblib.load('data/models/car_price_predictor_xgbr.pkl')
...
print(f"\n🎯 Predicted Price: {price_lakhs:.2f} lakhs")

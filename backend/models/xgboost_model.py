"""
XGBoost model implementation following Strategy Pattern
"""
...
class XGBoostModel(BaseModel):
    ...
    def __init__(self, model_path: str):
        ...
        """
        super().__init__(model_name="XGBoost", model_path=model_path)

    def load_model(self) -> bool:
        """
        Load XGBoost pipeline from pickle file
    ...
    def predict(self, input_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            # The model was trained on log1p(Price), so we use expm1 to reverse
    ...
    def get_feature_names(self) -> List[str]:
        ...
        The pipeline transforms 14 raw features into ~27 features:
            - 2 numerical (scaled): Engine (cc), Mileage_log
            - 7 binary (passthrough): Gear, Leasing, Condition, 4 features
            - ~18 categorical (one-hot): Brand_Grouped, Fuel_Grouped
    ...
    def get_model_info(self) -> Dict[str, Any]:
        ...
        if self.is_loaded:
            try:
                ...
                # Get XGBoost-specific parameters if available
                xgb_model = self.model.named_steps.get("model")
                if xgb_model:
                    info["hyperparameters"] = {
                        "n_estimators": getattr(xgb_model, "n_estimators", None),
                        "learning_rate": getattr(xgb_model, "learning_rate", None),
                        "max_depth": getattr(xgb_model, "max_depth", None),
                    }
            except Exception as e:
